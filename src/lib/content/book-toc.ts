import type { BookToc } from "@components/book-toc-types";

export const preHash = "#book/";

export const flipBookToc: BookToc["toc"] = [
  {
    headline: "0. Introduction",
    contents: [
      {
        title: "Editorial - P. Jankowski, A. Höfner & M. L. Hoffmann",
        url: `${preHash}6`,
        pageNumber: "006",
      },
      {
        title: "Preface - T. Santarius & N. Guenot",
        url: `${preHash}9`,
        pageNumber: "008",
      },
    ],
  },
  {
    headline:
      "1. Redefining Progress: Confronting the Challenges of Digitalisation and Sustainability",
    contents: [
      {
        title:
          "Challenges of Digitalisation and Sustainability at a Glance - Infographic - F. Rohde & P. Jankowski",
        url: `${preHash}14`,
        pageNumber: "014",
      },
      {
        title: "What Your Data Discloses About Others - L. Fröberg",
        url: `${preHash}15`,
        pageNumber: "015",
      },
      {
        title: "No Blockchains On A Dead Planet - P. Robben & F. Hildebrandt",
        url: `${preHash}21`,
        pageNumber: "021",
      },
      {
        title: "Artificial Intelligence for Real Sustainability - R. Rehak",
        url: `${preHash}26`,
        pageNumber: "026",
      },
      {
        title:
          "The Invisible Environmental Impact of Mobile Apps - F. Petri & M. Ruhenstroth",
        url: `${preHash}33`,
        pageNumber: "033",
      },
      {
        title:
          "Transparency for Software Climate Impact - S. Kruijer, A. Tarara & M. Schulze",
        url: `${preHash}38`,
        pageNumber: "038",
      },
    ],
  },
  {
    headline:
      "2. Values for Transformation: Foundation for a Socio-Ecological Digitalisation",
    contents: [
      {
        title:
          "A Feminist Reminder in Times of Digitalisation - H. Völkle & E. Lindinger",
        url: `${preHash}46`,
        pageNumber: "046",
      },
      {
        title:
          "How to Grow Alternative Platforms (Fast) - G. Franke & J. Pentzien",
        url: `${preHash}50`,
        pageNumber: "050",
      },
      {
        title: "An Office Without GAFAM? - V. Frick et al.",
        url: `${preHash}55`,
        pageNumber: "055",
      },
      {
        title: "Infrastructures as Commons - J. U. Hasecke & M. Hierweck",
        url: `${preHash}59`,
        pageNumber: "059",
      },
      {
        title: "The Digital Powerwash - B. Wijers",
        url: `${preHash}63`,
        pageNumber: "063",
      },
    ],
  },
  {
    headline:
      "3. Transformative Change: New Concepts and Paradigms to Update and Replace Current Socio-Technical Structures",
    contents: [
      {
        title:
          "Open Source Hardware and Open Design - L. Zimmermann & M. Voigt",
        url: `${preHash}68`,
        pageNumber: "068",
      },
      {
        title:
          "On Digital Sustainability and Digital Public Goods - M. Stürmer, M. Tiede, J. Nussbaumer & F. Wäspi",
        url: `${preHash}72`,
        pageNumber: "072",
      },
      {
        title: "From Digital Cannibalism to a New Food Pyramid - N. Rahman",
        url: `${preHash}77`,
        pageNumber: "077",
      },
      {
        title:
          "Business of Deep Transformations - I. Nesterova, L. Beyeler & L. Niessen",
        url: `${preHash}82`,
        pageNumber: "082",
      },
      {
        title:
          "Towards Artful Sustainable Integration of IT Infrastructures - E. Sørensen & S. Laser",
        url: `${preHash}87`,
        pageNumber: "087",
      },
    ],
  },
  {
    headline:
      "4. Beyond Business as Usual: Use Cases for More Sustainable Tech Design",
    contents: [
      {
        title:
          "Computer Scientists for Future - E. Eickstädt, M. Becke, M. Kohler & J. Padberg",
        url: `${preHash}94`,
        pageNumber: "094",
      },
      {
        title:
          "How Digitalisation Can Help Neighbourhoods Share Electricity - A. Aretz & I. Jungblut",
        url: `${preHash}99`,
        pageNumber: "099",
      },
      {
        title:
          "Digitalisation for a Socio-Ecological Transformation in Agriculture - L. Prause & A. Egger",
        url: `${preHash}104`,
        pageNumber: "104",
      },
      {
        title: "Workshop Results - melzai",
        url: `${preHash}110`,
        pageNumber: "110",
      },
    ],
  },
  {
    headline:
      "5. Politics Under Pressure: Necessary Changes on the Political Level",
    contents: [
      {
        title:
          "Bits & Bäume Political Demands at a Glance - Infographic - M. L. Hoffmann & J. Graf",
        url: `${preHash}114`,
        pageNumber: "114",
      },
      {
        title:
          "Making a More Sustainable Telecom Sector with Free Software - L. Lasota & E. Albers",
        url: `${preHash}116`,
        pageNumber: "116",
      },
      {
        title: "It's Time for Bold Policy-Making - H. Zimmermann & J. Pohl",
        url: `${preHash}120`,
        pageNumber: "120",
      },
      {
        title:
          "New Government for New Tasks? - S. Ramesohl, J. Wirtz, A. Gunnemann & R. Weier",
        url: `${preHash}123`,
        pageNumber: "123",
      },
    ],
  },
  {
    headline:
      "6. A Collective Effort: The Power of Civil Society Driving Sustainable Change",
    contents: [
      {
        title:
          "Why Designing a Sustainable Digital Future Requires Policy-Makers to Include Civil Society - V. Frick, A. Mollen & F. Rohde",
        url: `${preHash}130`,
        pageNumber: "130",
      },
      {
        title:
          "Preventing Surveillance Capitalism from Hijacking Socio-Ecological Transformations - L. Hennecke & K. Jung",
        url: `${preHash}133`,
        pageNumber: "133",
      },
      {
        title:
          "Crowdacting as a Spark for Climate Protection? - B. Parske & K. Kastner",
        url: `${preHash}138`,
        pageNumber: "138",
      },
      {
        title: "Hypernuding? - E. Lamura & M. Lamura",
        url: `${preHash}143`,
        pageNumber: "143",
      },
      {
        title: "How Can I Get Involved? Take Action! - N. Duddek & A. Höfner",
        url: `${preHash}148`,
        pageNumber: "148",
      },
    ],
  },
  {
    headline: "7. Closure",
    contents: [
      {
        title: "Acknowledgements",
        url: `${preHash}151`,
        pageNumber: "151",
      },
    ],
  },
];
